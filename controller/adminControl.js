...
const postSalesReport = async (req, res) => {
  try {
    ...
    console.log(new Date(req.body.startDate), ' start date in new date format')
    console.log(new Date(req.body.endDate), ' end date i');
    ...
    console.log(new Date(req.body.startDate) < new Date(req.body.endDate), ' status o fdate');
    const orders = await orderCollection.find({
